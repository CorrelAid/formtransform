"""XLSForm → DDI equivalence between formtransform and qwacback (formtransform#14).

For every answer type qwacback supports (qwacback/internal/examples/examples.go),
the same XLSForm goes through formtransform's `buildDdiXml` and qwacback's
`POST /api/convert/xlsform-to-ddi`, and the DDI shapes must match.
...
def _find_data_dscr(xml_str: str) -> ET.Element:
    ...
    formtransform always wraps in <codeBook>/<dataDscr>. qwacback returns a bare
...
def _var_shape(v: ET.Element) -> dict:
    ...
    cats = tuple((_text(_child(c, "catValu")), _text(_child(c, "labl"))) for c in _children(v, "catgry"))
    return {
        "name": v.get("name"),
        "ID": v.get("ID"),
        "intrvl": v.get("intrvl"),
...
class TestXlsformToDdiEquivalence:
    """formtransform and qwacback produce the same DDI for every supported type."""

    @pytest.mark.parametrize("label,survey,choices", EQUIVALENT_TYPES)
    def test_type_equivalence(self, formtransform_ddi, qwacback_ddi, label, survey, choices):
        our_vars, our_grps = _shape(formtransform_ddi(survey, choices))
        their_vars, their_grps = _shape(qwacback_ddi(survey, choices))
